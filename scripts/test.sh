...
DB_URL=$(grep '^DATABASE_URL=' /opt/pessoal/bibelocrm/.env | cut -d= -f2-)
DB_PASS=$(echo "$DB_URL" | sed -n 's|.*://[^:]*:\([^@]*\)@.*|\1|p')

# Carrega demais variáveis do .env (JWT_SECRET etc)
set -a
grep -v '^#' /opt/pessoal/bibelocrm/.env | grep -v '^$' | grep -v 'DATABASE_URL\|REDIS_URL\|DB_HOST' > /tmp/bibelo_test_env 2>/dev/null
...
REDIS_PASS=$(grep '^REDIS_PASS=' /opt/pessoal/bibelocrm/.env | cut -d= -f2-)

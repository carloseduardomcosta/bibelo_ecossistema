...
function getSafeReturnUrl(raw: string | null): string | null {
  if (!raw) return null
  // Só aceita paths internos (começa com /)
  if (raw.startsWith("/") && !raw.startsWith("//")) return raw
  return null
}

// ── Painel da conta (usuário logado) ──────────────────────────
function AccountPanel() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const { token, customer, logout } = useAuthStore()

  // Se veio com returnUrl (ex: redirecionado do checkout), vai direto
  useEffect(() => {
          ...
          <div className="flex flex-col items-center mb-6">
            {picture ? (
              <img
                src={picture}
                alt={displayName}
                className="w-20 h-20 rounded-full border-2 border-bibelo-pink/30 mb-3 object-cover"
...
function LoginForm() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const { token, customer, loading, setToken, loadCustomer } = useAuthStore()
  ...
  // Carrega customer se já tiver token
  useEffect(() => {
    if (token && !customer) {
      loadCustomer()
    }
  ...
  // Já logado → mostra AccountPanel (renderizado no componente pai)
  if (token && customer) return null
    ...
    try {
      if (mode === "login") {
        ...
        if (result.token) {
          ...
          await loadCustomer()
          router.replace(returnUrl || "/conta")
        }
      } else {
        const firstName = name.trim().split(" ")[0] || ""
        const lastName = name.trim().split(" ").slice(1).join(" ") || ""
        const token = await registerWithEmail(email, password, firstName, lastName)
        setToken(token)
        await loadCustomer()
        router.replace(returnUrl || "/conta")
      }
  ...
  const handleGoogleLogin = () => {
    ...
    setSubmitting(true)
    if (returnUrl) {
      try { sessionStorage.setItem(RETURN_URL_KEY, returnUrl) } catch {}
    }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="text-center mb-6">
            ...
            {returnUrl && (
              <p className="text-xs text-gray-400 mt-1 bg-gray-50 rounded-lg px-3 py-1.5">
...
function ContaPageInner() {
  ...
  if (token && customer) return <AccountPanel />
  return <LoginForm />
}
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center bg-[#FAF7F2]">
        <div className="w-8 h-8 border-2 border-bibelo-pink border-t-transparent rounded-full animate-spin" />
      </div>
    }>
      <ContaPageInner />
    </Suspense>

...
const MegaMenu = ({ categories, collections }: MegaMenuProps) => {
  ...
  return (
    <div className="hidden small:flex items-center gap-x-0 h-full">
      {/* Horizontal category links — estilo PROD uppercase */}
      {NAV_CATEGORIES.map((name) => {
        ...
        return (
          <LocalizedClientLink
            ...
            className="px-4 h-full flex items-center text-xs font-semibold uppercase tracking-widest text-bibelo-dark/80 hover:text-bibelo-pink hover:border-b-2 hover:border-bibelo-pink transition-all whitespace-nowrap"
          >
            {name}
          </LocalizedClientLink>
        )
      ...
      {/* "Todas as Categorias" dropdown com mega menu */}
      <Popover className="flex items-center h-full relative">
        {({ close }) => (
          <>
            <PopoverButton className="px-4 h-full flex items-center gap-x-1 text-xs font-semibold uppercase tracking-widest text-bibelo-dark/80 hover:text-bibelo-pink hover:border-b-2 hover:border-bibelo-pink transition-all outline-none whitespace-nowrap">
              Todas as Categorias
